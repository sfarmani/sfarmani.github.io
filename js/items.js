...
$(function () {
    ...
    $.when($.getJSON(items_url)).done(function (json) {
        items_json = json;
    });

    items_json = items_json.filter(x => !trolls.includes(x.type));
                ...
                render: function (data) {
                    ...
                    data.forEach(function (rec) {
                        if (Object.keys(rec).length > 1) {
                            let color0 = items_json.filter(x => x.name == Object.keys(rec)[0])[0].color
                            let color1 = items_json.filter(x => x.name == Object.keys(rec)[1])[0].color
                            ...
                            let color = items_json.filter(x => x.name == Object.keys(rec))[0].color
